<?php 
$sql_DanhMuc="SELECT TenDanhMuc FROM sanpham as s, danhmuc as d WHERE d.ID_DanhMuc = s.ID_DanhMuc";
$query_DanhMuc=mysqli_query($mysqli,$sql_DanhMuc);
$row_dm=mysqli_fetch_array($query_DanhMuc);
    if(isset($_GET['trang'])){
        $page = $_GET['trang'];
    } else{
        $page = 1;
    ...
    $sql_product="SELECT * FROM sanpham INNER JOIN danhmuc ON sanpham.ID_DanhMuc = danhmuc.ID_DanhMuc
    ORDER BY ID_SanPham DESC LIMIT $begin,8";
...
<div id="content" class="container-fluid">
    <div class="card">
        ...
        <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
            ...
            <div style="margin-right: 40px;" class="form-search form-inline">
                <form  action="" method="POST" class="d-flex">
                    <input type="text" class="form-control form-search" placeholder="Tìm kiếm" name="tukhoa">
                    <input type="submit" name="btn-search" value="Tìm kiếm" class="btn btn-danger" name="tim">
                </form>
            </div>
            
        </div>
        <div class="card-body">
        <div style="padding-bottom: 10px;right:0;"> <a class="btn btn-danger" href="?product=add-product">Thêm sản phẩm</a></div>
            <div class="tableInfo">
                <table class="table">
                    <thead>
                        <tr>
